...
function readSession(): Session {
  ...
  if (store) {
    ...
    if (raw) {
      try { return JSON.parse(raw) as Session; } catch {}
    }
  }
...
export const authApi: AuthService = {
  async signin(email, password) {
    const res = await fetch(`${AUTH_BASE}/signin`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) });
    if (!res.ok) { const msg = await res.text(); throw new Error(msg || 'AUTH_INVALID_CREDENTIALS'); }
  ...
  async signup(email, password) {
    const res = await fetch(`${AUTH_BASE}/signup`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) });
    if (!res.ok) { const msg = await res.text(); throw new Error(msg || 'AUTH_WEAK_PASSWORD'); }
    const data = await res.json();
    const token = data.access_token as string;
    setToken(token);
    const session: Session = { token, user: { id: 'me', email } as User };
